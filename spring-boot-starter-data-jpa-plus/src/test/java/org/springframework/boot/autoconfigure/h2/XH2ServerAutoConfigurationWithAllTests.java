...
/**
 * Tests for {@link XH2ServerAutoConfiguration} with all
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = { XH2ServerAutoConfiguration.class, DataSourceAutoConfiguration.class })
@ActiveProfiles({ "test", "test-h2-config-with-all" })
public class XH2ServerAutoConfigurationWithAllTests {
	...
	public void annotation() {
		...
		ConditionalOnProperty property = AnnotationUtils.findAnnotation(XH2ServerAutoConfiguration.class,
			ConditionalOnProperty.class);
		
		assertThat(property.prefix()).isEqualTo(XH2ServerProperties.PREFIX);
